...
//   // FIX: fetchQR now returns the image so checkStatus can use it directly
//   const fetchQR = async () => {
//     try {
//       const res = await axios.get(`${API_BASE_URL}/api/whatsapp/qr`);
//       console.log("QR response:", res.data); // debug — check what field the QR is in

//       // Handle both common response shapes: { qr: "..." } or { qrCode: "..." } or plain string
//       const raw = res.data?.qr || res.data?.qrCode || res.data?.image || res.data;

//       if (!raw) {
//         console.warn("QR data empty", res.data);
//         setQrImage(null);
//         return;
//       }

//       // Ensure it's a proper data URL for <img src>
//       const imgSrc = typeof raw === "string" && raw.startsWith("data:")
//         ? raw
//         : `data:image/png;base64,${raw}`;

//       setQrImage(imgSrc);
//     } catch (err) {
//       console.error("fetchQR failed:", err);
//       setQrImage(null);
//     }
//   };
...
//       console.log("Status:", res.data); // debug
//       setStatus(res.data);

//       if (res.data.hasQR && !res.data.connected) {
//         // Always fetch fresh QR when server says it's ready
//         await fetchQR();
...
//     // FIX: Separate intervals — status every 3s, QR every 5s
//     // This avoids hammering both endpoints at the same time (reduces reflow violations)
...
//         className="card"
//         style={{ padding: "18px 20px", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}
//       >
...
//         className="card"
//         style={{ padding: "18px 20px", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}
//       >
//         <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
//           <div style={{
//             width: 40, height: 40, borderRadius: 10,
...
//               style={{ fontSize: 12, padding: "5px 12px", color: "#c0392b", borderColor: "#c0392b" }}
//             >
...
//             className="card"
//             style={{ padding: 28, marginBottom: 12, textAlign: "center" }}
//           >
//             <p style={{ fontWeight: 600, fontSize: 15, marginBottom: 6, color: "var(--text-primary)" }}>
//               {status.hasQR ? "📲 Scan QR Code" : "⏳ Waiting for QR..."}
//             </p>
//             <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 24 }}>
//               {status.hasQR
//                 ? "Open WhatsApp → Linked Devices → Link a Device → Scan"
//                 : "QR code will appear once the server is ready. Auto-refreshes every 5 seconds."}
//             </p>
...
//                   onError={(e) => {
//                     console.error("QR image failed to load, src prefix:", qrImage?.slice(0, 50));
//                     e.target.style.display = "none";
//                   }}
//                   style={{
//                     width: 220, height: 220, borderRadius: 16,
//                     border: "2px solid var(--border)", padding: 12, background: "white",
...
//                 }} />
//                 {/* Manual refresh button in case polling lags */}
//                 <button
//                   onClick={fetchQR}
//                   style={{
...
//                     style={{ fontSize: 12, padding: "7px 14px", color: "#c0392b", borderColor: "#c0392b" }}
//                   >
